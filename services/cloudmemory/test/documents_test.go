...
func pickFirstDocumentID(t *testing.T, c *cloudmemory.Client) string {
	t.Helper()
	resp, err := c.ListDocuments(ctx(t), bankID(), cloudmemory.ListDocumentsOptions{Limit: 1})
	if err != nil {
...
func TestListDocuments(t *testing.T) {
	c := newClient(t)
	ensureBank(t, c)
	out, err := c.ListDocuments(ctx(t), bankID(), cloudmemory.ListDocumentsOptions{Limit: 10})
	if err != nil {
