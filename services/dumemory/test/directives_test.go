...
func TestDirectivesLifecycle(t *testing.T) {
	...
	t.Run("CreateDirective", func(t *testing.T) {
		req := dumemory.NewCreateDirectiveRequest("bce-sdk-it-directive", "always be concise")
	...
	t.Run("ListDirectives", func(t *testing.T) {
		out, err := c.ListDirectives(ctx(t), bankID(), dumemory.ListDirectivesOptions{Limit: 10})
		if err != nil {
